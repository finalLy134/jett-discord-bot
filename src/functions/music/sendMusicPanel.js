...
const musicpanel = require('../../commands/tools/musicpanel');
const GeniusClient = new Genius.Client(process.env.geniusSecret);

module.exports = (client) => {
    client.sendMusicPanel = async (queue, song) => {
        ...
        if (lyricsURL != undefined) {
            embed.setFooter({text: "Lyrics provided by GENIUS.com"});
        ...
        let musicPanel = await queue.textChannel.send({
            //content: `<@${song.user.id}>`,
            embeds: [embed],
            components: [controls, listControls, shortcutControls]
        });
    
        client.musicPanel = musicPanel;
        client.musicPanelID = musicPanel.id;
